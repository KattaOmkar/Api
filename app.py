...
class AddSpending(Resource):
    def post(self):
        try:
            ...
            #flask.app.logger.info('afdasdfas')
            # validate the received values
            if _date and _reason and _amount and request.method == 'POST':
                #do not save password as a plain text
                #_hashed_password = generate_password_hash(_password)
                # save edits
                sql = "INSERT INTO spending(date, reason, amount) VALUES(%s, %s, %s)"
    ...
    def post(self):
        try:
            ...
            if _date and _reason and _amount and request.method == 'POST':
                ...
                #_hashed_password = generate_password_hash(_password)
                # save edits
                sql = "INSERT INTO spending(date, reason, amount) VALUES(date(_date),_reason,_amount)"
                #data = (_date, _reason, _amount)
                #conn = mysql.connect()
                conn = db_connect.connect()
                cursor = db_connect.cursor()
